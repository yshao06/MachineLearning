...
test1.x <- testset1[-13]
train1.y <- trainset1[13]
validate1.y <- validateset1[13]
test1.y <- testset1[13]
# use knn.reg() function to predict median value of homes
k_list1 <- seq(1, 19, by = 2)
train1.MSE <- rep(0, length(k_list1))
validate1.MSE <- rep(0, length(k_list1))
for (i in k_list1) {
...
plot(NULL, NULL, type='n', xlim=c(length(k_list1)*2-1, 1), ylim=c(0,max(c(validate1.MSE, train1.MSE))), xlab='Increasing Flexibility (Decreasing k)', ylab='Mean Squared Errors', main='MSEs as a Function of \n Flexibility for KNN Regression')
lines(seq(length(k_list1)*2-1, 1, by = -2), validate1.MSE[length(validate1.MSE):1], type='b', col=2, pch=16)
lines(seq(length(k_list1)*2-1, 1, by = -2), train1.MSE[length(train1.MSE):1], type='b', col=1, pch=16)
...
train2.error <- rep(0, length(k_list2))
validate2.error <- rep(0, length(k_list2))
for (i in k_list2) {
...
plot(NULL, NULL, type='n', xlim=c(length(k_list2)*2-1, 1), ylim=c(0,max(c(validate2.error, train2.error))), xlab='Increasing Flexibility (Decreasing k)', ylab='Error Rates', main='Error Rates as a Function of \n Flexibility for KNN Classification')
lines(seq(length(k_list2)*2-1, 1, by = -2), validate2.error[length(validate2.error):1], type='b', col=2, pch=16)
lines(seq(length(k_list2)*2-1, 1, by = -2), train2.error[length(train2.error):1], type='b', col=1, pch=16)
...
for (j in 1:50) {
    ...
    validate3.MSE <- rep(0, length(k_list1))
    for (i in k_list1) {
...
### Therefore, conclude that the searching of best k value through training & validate dataset
...
legend("topright", legend = c("Validation MSEs", "Validation MSE mean", "Test MSEs", "Test MSE mean"), col=c(2, 2, 1, 1), cex=.50, pch=c(16, NA, 16, NA), lty=c(1, 2, 1, 2))
...
app_adj <- app[, c(-5, -7, -17)]
n4 <- nrow(app_adj)
#str(app_adj)

set.seed(5072)
best.validate4.MSE <- matrix(rep(0, 100), ncol = 2)
...
#        train4.MSE[(i+1)/2] <- sum((knn4.pred - train4.y)^2)/length(knn4.pred)
    ...
    best.validate4.MSE[j, 1] <- validate4.MSE[which.min(validate4.MSE)]
    best.validate4.MSE[j, 2] <- k_list4[which.min(validate4.MSE)]
}
# Print minimum validate/training MSE and corresponding k's
print(paste("Minimum validate set MSE occurred at k=", best.validate4.MSE[which.min(best.validate4.MSE[, 1]), 2]))
print(paste("Minimum validate MSE was", best.validate4.MSE[which.min(best.validate4.MSE[, 1]), 1]))

# The flexibility producing best MSE is k=5
# The following statement is to omit variable "Book.Costs", "Percent.Donating.Alumni", and "Enrolled"
app <- read.csv(file = "applications.train.csv", sep = ",", stringsAsFactors = F)
app <- app[, c(-5, -7, -17)]
